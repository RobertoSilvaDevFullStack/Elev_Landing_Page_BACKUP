<?php
/**
 * API Lead Backup - Versão Final Corrigida
 * Recebe dados do React e salva no MySQL + envia email
 * Sem conflitos de constantes
 */
...
error_log("=== LEAD BACKUP API FINAL START ===");
...
try {
    ...
    // Usar email service corrigido
    if (!file_exists('email-service-fixed.php')) {
        throw new Exception("Arquivo email-service-fixed.php não encontrado");
    }
    ...
    try {
        ...
        try {
            ...
            error_log("⚠️ Erro no email (não crítico): " . $emailError->getMessage());
        }
        ...
        echo json_encode([
            ...
            'data_saved' => [
                ...
                'email' => $mysqlData['email'],
                'phone' => $mysqlData['phone'],
                'interest' => $mysqlData['interest_type']
            ]
        ]);
...
error_log("=== LEAD BACKUP API FINAL END ===");
?>
